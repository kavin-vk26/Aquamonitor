...
def main():
    try:
        ...
        model_candidates = [
            ...
            os.path.join(os.path.dirname(__file__), 'yolo.tflite'),
        ]
        ...
        if not model_path:
            raise Exception('Model file not found. Please ensure third_yolo.tflite is in the same directory.')
        ...
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            ...
            # Parse predictions with lower threshold for better detection
            boxes, scores, classes = parse_predictions(output_data, input_shape, score_thresh=0.1)

        # If no detections, return no predator
        ...
        if len(boxes) > 1:
            ...
            if keep:
                ...
                best_idx = keep[0]
                top_cls = int(classes[best_idx])
                top_conf = float(scores[best_idx])
            else:
                # If NMS removes all, take the highest scoring one
                best_idx = np.argmax(scores)
                top_cls = int(classes[best_idx])
                top_conf = float(scores[best_idx])
        else:
            # Single detection
            top_cls = int(classes[0])
            top_conf = float(scores[0])

        # Map class ID to predator name
        predator_name = class_names.get(top_cls, f"Unknown Predator (Class {top_cls})")
        ...
        if top_conf < 0.1:
            ...
            result = {
                ...
                'confidence': min(top_conf, 0.99)  # Cap confidence at 99%
            }
